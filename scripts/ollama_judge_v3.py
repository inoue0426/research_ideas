...
def call_ollama(url, model, prompt, timeout, output_format):
    ...
    try:
        ...
        raise RuntimeError(
            f"Ollama HTTP request failed: "
            f"status={exc.code}, model={model!r}, url={url!r}, body={body!r}"
        ) from exc

    except urllib.error.URLError as exc:
        raise RuntimeError(
            f"Could not connect to Ollama at {url!r}: {exc}"
        ) from exc
...
def validate_and_translate(text, pairs):
    ...
    if set(returned) != set(by_id):
        return None, (
            f"expected pair IDs {sorted(by_id)}, "
    ...
    for pair in pairs:
        ...
        if winner_issue == 0:
            ...
            return None, (
                f"{pair['pair_id']}: winner_issue {winner_issue} "
                f"is not one of the paired issues #{a}/#{b} or 0"
            )

        if winner_issue != 0 and f"#{winner_issue}" not in reason:
            return None, (
                f"{pair['pair_id']}: reason must explicitly name "
                f"winning issue #{winner_issue}; got {reason!r}"
            )
...
def main():
    ...
    parser.add_argument(
        "--url",
        default=os.environ.get("OLLAMA_URL", DEFAULT_URL),
    )
...
"""
    ...
    for attempt in range(1, args.retries + 1):
        ...
        print(
            f"Ollama output consistency validation failed "
            f"on attempt {attempt}: {last_error}"
        )

    raise RuntimeError(
        "Ollama failed to produce consistent pairwise output "
