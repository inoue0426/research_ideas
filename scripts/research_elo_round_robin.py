...
"""Helpers for complete round-robin Research Elo bootstrapping.

The bootstrap schedule contains every unordered pair exactly once. Pair order is
shuffled deterministically so the online Elo update order is not correlated with
GitHub issue number. Batches keep one target issue plus up to five opponents so
they remain compatible with the existing Research Elo prompt/apply pipeline.
"""
...
def main():
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        ...
        f.write("\n")
    print(
        f"Round robin: {len(schedule['issues'])} issues, "
        f"{schedule['pair_count']} unique pairs, {len(schedule['batches'])} batches"
    )
